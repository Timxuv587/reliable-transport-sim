...
class Streamer:
    ...
    def send(self, data_bytes: bytes) -> None:
        ...
        while length - index > 1472:
            header = "Seq: " + str(sequence_num) + " "
            data = data_bytes[index:index+1472]
            new_data = struct.pack(header.encode("utf-8"),data)
            self.socket.sendto(new_data, (self.dst_ip, self.dst_port))
            index += 1472
            sequence_num += 1
        header = "Seq: " + str(sequence_num) + " "
        new_data = struct.pack(header.encode("utf-8"), data_bytes[index:length])
        self.socket.sendto(new_data, (self.dst_ip, self.dst_port))
    ...
    def recv(self) -> bytes:
        ...
        data = []
        for i in self.seq.sort():
            data.append(recv_buffer)
